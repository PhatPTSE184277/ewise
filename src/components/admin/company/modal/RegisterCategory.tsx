...
import { useCompanyCategoryContext } from '@/contexts/admin/CompanyCategoryContext';
...
const RegisterCategory: React.FC<RegisterCategoryProps> = ({
  ...
  const handleToggleSubcategories = async (parentId: string) => {
      ...
      >
        <div className='flex justify-between items-center p-6 border-b border-primary-100 bg-linear-to-r from-primary-50 to-primary-100'>
          <h2 className='text-2xl font-bold text-gray-900'>
            {currentCategoryIds.length > 0 ? 'Cập nhật danh mục' : 'Đăng ký danh mục'}
          </h2>
          <button
            onClick={handleClose}
                    ...
                    {parentCategories.map((parent, index) => {
                      ...
                      return (
                        <React.Fragment key={parent.id}>
                          <tr className={`border-b border-primary-100 ${rowBg} transition-colors`}>
                          ...
                          {isExpanded && (
                            <tr className='bg-white'>
                              <td colSpan={3} className='px-6 py-4 border-b border-primary-100'>
                                    ...
                                    <table className='w-full text-sm'>
                                      ...
                                      <tbody>
                                        {subcategories.map((sub, subIndex) => {
                                          const subRowBg = subIndex % 2 === 0 ? 'bg-white' : 'bg-primary-50';
                                          return (
                                            <tr
                                              key={sub.id}
                                              className={`${subIndex !== subcategories.length - 1 ? 'border-b border-primary-100' : ''} ${subRowBg}`}
                                            >
